...
namespace Microsoft.Teams.Apps.NewHireOnboarding.Authentication
{
    ...
    public static class AuthenticationServiceCollectionExtensions
    {
        ...
        public static void AddNewHireOnboardingAuthentication(
        ...
        /// </summary>
        /// <param name="configuration">Represents a set of key/value application configuration properties.</param>
        /// <param name="validIssuerKey">A valid issuer key.</param>
        /// <returns>A collection of configuration setting.</returns>
        private static IEnumerable<string> GetValidIssuersConfigurationSettings(IConfiguration configuration, string validIssuerKey)
        {
            var configurationSettingsValue = configuration[validIssuerKey];
            var settings = configurationSettingsValue
                ?.Split(new char[] { ';', ',' }, StringSplitOptions.RemoveEmptyEntries)
                ?.Select(p => p.Trim());

            settings = settings ?? throw new ApplicationException($"{validIssuerKey} does not contain a valid value in the configuration file.");
        ...
        private static IEnumerable<string> GetValidIssuers(IConfiguration configuration)
        {
            ...
            var validIssuers =
                AuthenticationServiceCollectionExtensions.GetValidIssuersConfigurationSettings(
                    configuration,
        ...
        private static void RegisterAuthorizationPolicy(IServiceCollection services)
        {
            ...
            {
                options.AddPolicy(
                    PolicyNames.MustBeHumanResourceTeamMemberUserPolicy,
                    policyBuilder => policyBuilder.AddRequirements(new MustBeHumanResourceTeamMemberRequirement()));
            });

            services.AddSingleton<IAuthorizationHandler, MustBeHumanResourceTeamMemberHandler>();
        }
    }
}
